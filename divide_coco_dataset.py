...
file_prefix = "coco_2017_val_split"

# データセットファイルのパス
instances_file_path = 'dataset/coco/annotations/instances_val2017.json'
...
for image_id, categories in image_categories.items():
    ...
    whole_category_images[image_id] = convert_to_coco_ids(list(categories))
    ...
    if not categories:
        ...
        if contains_a:
            ...
            common_only_images[image_id] = convert_to_coco_ids(list(categories))
            dataset_a_images[image_id] = convert_to_coco_ids(list(categories))
            dataset_b_images[image_id] = convert_to_coco_ids(list(categories))
            dataset_a_categories.update(categories & categoryC_set)
            dataset_b_categories.update(categories & categoryC_set)
    elif contains_a:
        category_a_only_images[image_id] = convert_to_coco_ids(list(categories))
        dataset_a_images[image_id] = convert_to_coco_ids(list(categories))
        dataset_a_categories.update(categories & categoryA_set)
    elif contains_b:
        category_b_only_images[image_id] = convert_to_coco_ids(list(categories))
        dataset_b_images[image_id] = convert_to_coco_ids(list(categories))
...
category_a_only_images_info = {
    "categories": {mapped_index_to_category_id[cat_id]: category_id_to_name[mapped_index_to_category_id[cat_id]] for cat_id in categoryA_set},
    "image_ids": category_a_only_images,
}

category_b_only_images_info = {
    "categories": {mapped_index_to_category_id[cat_id]: category_id_to_name[mapped_index_to_category_id[cat_id]] for cat_id in categoryB_set},
    "image_ids": category_b_only_images,
}

common_only_images_info = {
    "categories": {mapped_index_to_category_id[cat_id]: category_id_to_name[mapped_index_to_category_id[cat_id]] for cat_id in categoryC_set},
    "image_ids": common_only_images,
}

whole_category_images_info = {
    "categories": {mapped_index_to_category_id[cat_id]: category_id_to_name[mapped_index_to_category_id[cat_id]] for cat_id in all_categories},
...
with open(os.path.join(save_dir, f"{file_prefix}_category_a_only_images_info.json"), "w") as f:
    json.dump(category_a_only_images_info, f, indent=4)
print(f"データセットAの独自カテゴリーの画像情報を '{file_prefix}_category_a_only_images_info.json' に保存しました。")

with open(os.path.join(save_dir, f"{file_prefix}_category_b_only_images_info.json"), "w") as f:
    json.dump(category_b_only_images_info, f, indent=4)
print(f"データセットBの独自カテゴリーの画像情報を '{file_prefix}_category_b_only_images_info.json' に保存しました。")

with open(os.path.join(save_dir, f"{file_prefix}_common_only_images_info.json"), "w") as f:
    json.dump(common_only_images_info, f, indent=4)
print(f"共通カテゴリーのみを含む画像情報を '{file_prefix}_common_only_images_info.json' に保存しました。")

with open(os.path.join(save_dir, f"{file_prefix}_whole_category_images_info.json"), "w") as f:
    json.dump(whole_category_images_info, f, indent=4)
print(f"すべてのカテゴリーの画像情報を '{file_prefix}_whole_category_images_info.json' に保存しました。")
