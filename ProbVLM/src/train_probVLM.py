    ...
    for eph in range(num_epochs):
        ...
        with tqdm(train_loader, unit='batch') as tepoch:
            for (idx, batch) in enumerate(tepoch):
                ...
                ##
                xI, xT, index  = batch[0].to(device), batch[1].to(device), batch[-1]

                if w is not None:
                    xT = w[index]

                with torch.no_grad():
                    # xfI, xfT = CLIP_Net.forward(xI, xT)
                ...
                if idx % 500 == 0:
                    torch.save(BayesCap_Net.state_dict(), ckpt_path+f'_last_{save_name}.pth')

            eph_loss /= len(train_loader)
            all_loss.append(eph_loss)
            print('Avg. loss: {}'.format(eph_loss))
        # evaluate and save the models
        torch.save(BayesCap_Net.state_dict(), ckpt_path+f'ProbVLM_Net_last_{save_name}.pth')
        if eph%eval_every == 0:
            ...
            if curr_score <= score:
                score = curr_score
                torch.save(BayesCap_Net.state_dict(), ckpt_path+f'ProbVLM_Net_best_{save_name}.pth')
