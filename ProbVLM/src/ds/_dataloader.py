...
from ProbVLM.src.ds.coco import CocoCaptionsCap, CocoBboxes
# from ds.vocab import Vocabulary
from ProbVLM.src.ds.vocab import Vocabulary
#from datasets._transforms import imagenet_transform
# from ds._transforms import  caption_transform
from ProbVLM.src.ds._transforms import  caption_transform
...
def _get_coco_file_paths(dataset_root):
    """Select proper train / val classes and omit id files.
    """
    train_ids = np.load('ProbVLM/src/ds/annotations/coco_train_ids.npy')
    train_extra_ids = np.load('ProbVLM/src/ds/annotations/coco_restval_ids.npy')
    val_ids = np.load('ProbVLM/src/ds/annotations/coco_dev_ids.npy')[:5000]
    te_ids = np.load('ProbVLM/src/ds/annotations/coco_test_ids.npy')
