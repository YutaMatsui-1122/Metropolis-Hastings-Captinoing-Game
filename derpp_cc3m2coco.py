...
agent_cc3m = OneAgent(agent_name='A', device=device, td_update_epochs=args.td_update_epochs, temperature=0.63, clip_arch=args.pretrain_arch, td_train_mode="DERPP", td_alpha_beta=args.alpha_beta, pretrain=True)
agent_cc3m = agent_cc3m.to(device)
agent_cc3m.save_dir = f"models/{args.save_dir}"
agent_coco = OneAgent(agent_name='A', device=device, td_update_epochs=args.td_update_epochs, temperature=0.63, clip_arch=speaker_arch, td_train_mode="DERPP", td_alpha_beta=args.alpha_beta)
...
if args.pretrain_arch == "ViT-B/32":
    agent_coco.load_pretrain(probvlm_path="models/official_model/probvlm/CC3M/probvlm_0.2_0.2_20_arch_ViT-B-16-epoch-45.pth", clipcap_path="models/clipcap_vit16_cc3m/clipcap_019.pt", strict_clipcap=False)
elif args.pretrain_arch == "ViT-B/16":
...
if "debug" in args.save_dir:
    ...
    if args.pretrain_arch == "ViT-B/32":
        pretrain_train_file = "coco_train_dataset_10000"
    elif args.pretrain_arch == "ViT-B/16":
...
agent_coco.communication_field_setup(finetune_train_dataloader_fix, finetune_train_dataloader, MH_iter=10, EM_iter=10)
agent_coco.perception()
agent_cc3m.communication_field_setup(finetune_train_dataloader_fix, finetune_train_dataloader, MH_iter=10, EM_iter=10)
...
for epoch in range(10):
    ...
    agent_cc3m.update_text_decoder(epoch)