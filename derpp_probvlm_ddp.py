...
from update_models import update_probvlm_derpp_ddp
from utils import * 
...
def main(rank, world_size):
    # cuda:1, cuda:2, cuda:3の使用に変更
    gpu_map = {0: 1, 1: 2, 2: 3}
    device = f"cuda:{gpu_map[rank]}"  # rankに応じたGPUのマッピング
    ...
    # Agentの初期化
    agentA = OneAgent(agent_name='A', device=device, td_update_epochs=args.td_update_epochs, te_update_epochs=args.te_update_epochs, temperature=args.temperature, der_alpha=args.agentA_alpha, derpp_beta=args.agentA_beta)
    ...
    # DDPで学習を実行
    probvlm = update_probvlm_derpp_ddp(rank, world_size, agentA.z, agentA.CLIP_Net, agentA.ProbVLM_Net, agentA.dataloader_MHNG_shuffle, agentA.save_dir, epochs=agentA.te_update_epochs, lr=1e-5, device=device, output_prefix="probvlm_"+agentA.agent_name, save_every=1, buffer=agentA.te_buffer, alpha=agentA.der_alpha, beta=agentA.derpp_beta, train_mode=args.mode)

# メイン関数
if __name__ == '__main__':
    world_size = 3  # cuda:1, cuda:2, cuda:3 の3つのGPUを使用
