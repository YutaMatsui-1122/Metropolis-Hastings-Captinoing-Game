...
# coco_test_dataset_A = CocoDataset(root = data_path, transform=preprocess,data_mode="train", prefix_length=prefix_length, normalize_prefix=normalize_prefix, datasize=100)
train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=32, shuffle=True, num_workers=args.num_workers)
test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=32, shuffle=False, num_workers=args.num_workers)
...
def pretrain_clipcap(CLIP_Net, train_loader, test_loader, model, save_dir, epochs, lr = 2e-8, warmup_steps_percent = 0.1, output_prefix = "clipcap", save_every = 1, train_mode = "pretain", initial_epoch = 0, r=2):
    device = torch.device('cuda:3' if torch.cuda.is_available() else 'cpu')
    peft_config = LoraConfig(task_type=TaskType.CAUSAL_LM, inference_mode=False, r=r, lora_alpha=32, lora_dropout=0.1, target_modules=["c_fc"])
    ...
    for epoch in range(initial_epoch+1, epochs):
        ...
        for idx, batch in enumerate(train_loader):
            ...
            print(gpt_token.shape, prefix.shape, gpt_mask.shape)
    ...
    # save ddd time in txt

    with open(os.path.join(save_dir, f"{output_prefix}_time.txt"), "w") as f:
...
for r in [2]:
    ...
    # make directory in cc3m2coco
    os.makedirs(f"models/{r}", exist_ok=True)
    model = pretrain_clipcap(clip_model, train_dataloader, test_dataloader, agent.ClipCap, f"models/{r}", 50, lr = 2e-7, warmup_steps_percent = 0.1, output_prefix = "clipcap", save_every = 1, train_mode = "pretrain", initial_epoch = args.initial_epoch, r=r)